...
pub fn expand_derive_entity_model(
    ...
    let mut computed_fields: Option<String> = None;
        ...
        .try_for_each(|attr| {
            attr.parse_nested_meta(|meta| {
                if meta.path.is_ident("comment") {
                    ...
                    model_ex = true;
                } else if meta.path.is_ident("computed_fields") {
                    computed_fields = Some(meta.value()?.parse::<LitStr>()?.value());
    ...
    let entity_def = table_name
        .as_ref()
        .map(|table_name| {
            let cf_meta = computed_fields.as_ref().map(|cf| {
                let cf_lit = syn::LitStr::new(cf, Span::call_site());
                quote!(computed_fields = #cf_lit)
            });
            // Emit a single `#[sea_orm(...)]` attribute carrying the ModelEx
            // wiring and/or the computed-fields declaration. Keeping these in
            // one attribute is required so the attribute derive reliably reads
            // them together (multiple separate `sea_orm` attributes are not all
            // honoured).
            let entity_extra_attr = match (model_ex, &cf_meta) {
                (true, Some(cf)) => {
                    quote!(#[sea_orm(model_ex = ModelEx, active_model_ex = ActiveModelEx, #cf)])
                }
                (true, None) => {
                    quote!(#[sea_orm(model_ex = ModelEx, active_model_ex = ActiveModelEx)])
                }
                (false, Some(cf)) => quote!(#[sea_orm(#cf)]),
                (false, None) => quote!(),
            };
            quote! {
